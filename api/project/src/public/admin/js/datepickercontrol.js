...
DatePickerControl.getDaysCode = function()
{
	...
	var onlyDay = inputElement.getAttribute("day");
	var vDate = new Date();
	vDate.setDate(1);
	...
	for (j=vFirstDay-this.firstWeekDay; j<7; j++) {
		...
		if (j==onlyDay || onlyDay==null)
		{
		vCode = vCode + "<td class='" + classname + "' class_orig='" + classname + "' " + "onClick='DatePickerControl.writeDate(" + vDay + ")' id='" + this.dayIdPrefix + vDay + "'>" + vDay +	"</td>";
		}
		else
		{
		vCode = vCode + "<td class='dayothermonth'>" + vDay + "</td>";			
		}
		vDay = vDay + 1;
	}
	vCode = vCode + "</tr>";
